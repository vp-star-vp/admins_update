<template>
  ...
  >
    <div class="px-3 py-2">
      ...
      <b-card-text>
        <span>Выбери разрешние и нажми кнопку <b>добавить</b>.  </span>
        ...
        <b-row>
          <b-col cols="8">
            <v-select
              ...
              :options="$store.getters['userManagement/permissions/getSelectPermission']"
              label="Name"
            />
          </b-col>
          <b-col cols="4">
...
export default {
  ...
  methods: {
    ...
    async selectApp() {
      await store.dispatch('userManagement/permissions/permissionList2LookUp', { url: store.getters.USER_STATE_FULL.url, id: this.selectedSite.solutionId })
    },
    add() {
